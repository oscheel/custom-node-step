...
public class CustomStepExecution extends AbstractStepExecutionImpl implements Serializable{


    private final CustomStep step;

    CustomStepExecution(StepContext ctx,CustomStep step){
        super(ctx);
        this.step = step;
    }
        ...
        @Override public void onLeft(Queue.LeftItem li) {
            if (li.isCancelled()) {
                if (li.task instanceof PlaceholderTask) {
                    (((PlaceholderTask) li.task).context).onFailure(new AbortException("Task was cancelled."));
                }
            }
        }
    ...
    public static final class PlaceholderTask implements ContinuedTask, Serializable, AccessControlled {
        ...
        /** Initially set to {@link CustomStep#getLabel}, if any; later switched to actual self-label when block runs. */
        ...
        @Override public String getDisplayName() {
            ...
            if (r != null) {
                ...
                if (enclosingLabel != null) {
                    return runDisplayName + " "+ enclosingLabel;
                } else {
                ...
                Computer.threadPoolForRemoting.submit(new Runnable() { // JENKINS-34542, JENKINS-45553
                    ...
                    public void run() {
                        try {
                            ...
                            LOGGER.log(WARNING, "failed to shut down " + cookie, x);
                        }
                    }
                });
            }
        ...
        private static final long serialVersionUID = 109888558037315599L;
    }
    ...
    private static final Logger LOGGER = Logger.getLogger(CustomStepExecution.class.getName());
}
